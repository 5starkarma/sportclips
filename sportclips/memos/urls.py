...
urlpatterns = [
    path('', MemoListView.as_view(template_name='memos/memos.html'), name='memos-memos'),
    path('<int:pk>', MemoDetailView.as_view(), name='memos-detail'),
    path('user/<str:username>', UserMemoListView.as_view(), name='user-memos'),
    ...
    path('<int:pk>/comment/', views.add_comment_to_memo, name='add-comment-to-memo'),
]
